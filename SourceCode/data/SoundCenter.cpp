...
/**
 * @brief The update function searches all sample instances and destroy
 * instances that have finished playing.
 * @details An instance that has finished playing needs to satisfy all the
    ...
    for (auto &[path, audio_pair] : samples) {
      ...
      for (auto it = insts.begin(); it != insts.end();) {
        if (al_get_sample_instance_playing(*it))
          ++it;
        else if (al_get_sample_instance_position(*it) != 0)
          ++it;
        else if (al_get_sample_instance_playmode(*it) == ALLEGRO_PLAYMODE_LOOP)
          ++it;
        else {
 ...
 * @details This function will also destroy all sample instances played by the
 * sample to be destroyed.
 * @param path audio path.
 * @return True if the sample of the path is destroyed. False if the sample does
 * not exist.
 */
bool SoundCenter::erase_sample(const std::string &path) {
 ...
 * @details For the list of supported play modes, refer to
 * [manual](https://liballeg.org/a5docs/trunk/audio.html#allegro_playmode).
 */
ALLEGRO_SAMPLE_INSTANCE *SoundCenter::play(const string &path,
...
void SoundCenter::toggle_playing(ALLEGRO_SAMPLE_INSTANCE *inst) {
  ...
  if (is_playing) {
    ...
    // As the sample stops, allegro will automatically reset the play position
    // to 0. We need to set it back to be able to resume.
