 ...
 * @details The globally used data such as FPS (frames per second), windows
 * size, game region, and states of input devices (mouse and keyboard). Player
 * and Level are stored here mainly because the objects are small enough that do
 * not require complex management. Other specific data like game objects
 * (towers, monsters ... etc) or ALLEGRO_BITMAP will be managed by other center
 * objects.
 */
class DataCenter {
public:
...
public:
  ...
  /**
   * @brief The width and height of game area (not window size). That is, the
   * region excludes menu region.
   ...
   * @details The states will be updated once a key is pressed, asynchronously
   * with frame update.
   * @see Game::execute()
   */
...
public:
   ...
   * @details For a tower-defense game, coin and health point is enough to
   * represent a player.
   * @see Player
   */
  Player *player;
