...
void UI::init() {
  ...
  for (size_t i = 0; i < (size_t)(TowerType::TOWERTYPE_MAX); ++i) {
    ...
    tower_items.emplace_back(bitmap, Point{tl_x, tl_y},
                             TowerSetting::tower_price[i]);
...
void UI::update() {
  ...
  case STATE::HOVER: {
    ...
    Tower::get_bitmap(static_cast<TowerType>(on_item)); int w =
    al_get_bitmap_width(bitmap); int h = al_get_bitmap_height(bitmap); Rectangle
    place_region{mouse.x - w / 2, mouse.y - h / 2, DC->mouse.x + w / 2,
    DC->mouse.y + h / 2}; bool place = true;
            ...
            if(!place) {
                    ...
                    DC->towers.emplace_back(Tower::create_tower(static_cast<TowerType>(on_item),
    mouse)); DC->player->coin -= std::get<2>(tower_items[on_item]);
            }
            debug_log("<UI> state: change to HALT\n");
            state = STATE::HALT;
            break;
