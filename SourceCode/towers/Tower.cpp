...
ALLEGRO_BITMAP *Tower::get_bitmap(TowerType type) {
  ImageCenter *IC = ImageCenter::get_instance();
  return IC->get(TowerSetting::tower_full_img_path[static_cast<int>(type)]);
}

Tower *Tower::create_tower(TowerType type, const Point &p) {
  switch (type) {
 ...
 */
Tower::Tower(const Point &p, double attack_range, int attack_freq,
             TowerType type) {
  ImageCenter *IC = ImageCenter::get_instance();
  // shape here is used to represent the tower's defending region. If any
  // monster walks into this area (i.e. the bounding box of the monster and
  // defending region of the tower has overlap), the tower should attack.
 ...
 */
Rectangle Tower::get_region() const {
  int w = al_get_bitmap_width(bitmap);
  int h = al_get_bitmap_height(bitmap);
  return {shape->center_x() - w / 2, shape->center_y() - h / 2,
          shape->center_x() - w / 2 + w, shape->center_y() - h / 2 + h};
}
